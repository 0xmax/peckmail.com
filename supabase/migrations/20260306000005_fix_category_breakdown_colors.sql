-- Fix category breakdown colors so dashboards respect enum colors and use the same fallback palette as the UI.
...
CREATE OR REPLACE FUNCTION resolve_email_category_color(
  p_enum_values TEXT[],
  p_enum_colors TEXT[],
  p_value TEXT
)
...
AS $$
  ...
  normalized AS (
    SELECT normalize_email_enum_token(p_value) AS value_token
  ),
  matched AS (
    SELECT ev.ord
    FROM unnest(COALESCE(p_enum_values, ARRAY[]::TEXT[])) WITH ORDINALITY AS ev(value, ord)
    CROSS JOIN normalized n
    WHERE ev.value = p_value
       OR normalize_email_enum_token(ev.value) = n.value_token
    ORDER BY CASE WHEN ev.value = p_value THEN 0 ELSE 1 END, ev.ord
    LIMIT 1
  )
  SELECT CASE
    WHEN NULLIF(trim(COALESCE(p_value, '')), '') IS NULL THEN '#94a3b8'
    ELSE COALESCE(
      (COALESCE(p_enum_colors, ARRAY[]::TEXT[]))[(SELECT ord::int FROM matched)],
      (
        SELECT colors[
          1 + (
            MOD(
              (GREATEST(COALESCE((SELECT ord FROM matched), 1), 1) - 1)::bigint,
